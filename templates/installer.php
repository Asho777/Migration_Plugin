...
class WordPressMigrationInstaller {
    ...
    private function step_database_config() {
        ...
        <form method="post">
            ...
            <div class="form-group">
                ...
                <input type="text" id="table_prefix" name="table_prefix" value="<?php echo isset($_POST['table_prefix']) ? htmlspecialchars($_POST['table_prefix']) : (isset($this->config['table_prefix']) ? $this->config['table_prefix'] : 'wp_'); ?>" required>
            </div>
    ...
    private function create_wp_config($config) {
        ...
        $wp_config_content .= "define('DB_NAME', '" . addslashes($config['name']) . "');\n";
        $wp_config_content .= "define('DB_USER', '" . addslashes($config['user']) . "');\n";
        $wp_config_content .= "define('DB_PASSWORD', '" . addslashes($config['pass']) . "');\n";
        $wp_config_content .= "define('DB_HOST', '" . addslashes($config['host']) . "');\n";
        ...
        $wp_config_content .= "\$table_prefix = '" . addslashes($config['prefix']) . "';\n\n";
    ...
    private function import_database($config) {
        ...
        if (isset($this->config['table_prefix']) && $this->config['table_prefix'] !== $config['prefix']) {
            $sql_content = str_replace($this->config['table_prefix'], $config['prefix'], $sql_content);
        }
    ...
    private function update_urls($config) {
        ...
        $queries = array(
            "UPDATE {$prefix}options SET option_value = '" . mysqli_real_escape_string($connection, $new_url) . "' WHERE option_name = 'home'",
            "UPDATE {$prefix}options SET option_value = '" . mysqli_real_escape_string($connection, $new_url) . "' WHERE option_name = 'siteurl'",
            "UPDATE {$prefix}posts SET post_content = REPLACE(post_content, '" . mysqli_real_escape_string($connection, $old_url) . "', '" . mysqli_real_escape_string($connection, $new_url) . "')",
            "UPDATE {$prefix}postmeta SET meta_value = REPLACE(meta_value, '" . mysqli_real_escape_string($connection, $old_url) . "', '" . mysqli_real_escape_string($connection, $new_url) . "')",
            "UPDATE {$prefix}comments SET comment_content = REPLACE(comment_content, '" . mysqli_real_escape_string($connection, $old_url) . "', '" . mysqli_real_escape_string($connection, $new_url) . "')"
        );
